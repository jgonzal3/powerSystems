...
def calculate_ai(M, lambda_value, B, K):
    ai = [(M[i][0] + lambda_value*B[i][i])/(1-K[i])**2 for i in range(len(B))]
    return ai

def  calculate_bi(M, lambda_value, B, K, power_values):
    bi = [(M[i][1] - 2*lambda_value*B[i][i]*power_values[i])/(1 - K[i]) for i in range(len(M))]
    return bi
...
def perform_load_flow(PD,M,Bij, param, lambda_value,power_values):
    ...
    ai_star = calculate_ai(M, lambda_value, Bij, K)
    bi_star = calculate_bi(M, lambda_value, Bij, K, power_values)
    PD = PD + PL - sum(power_values[i] * K[i] for i in range(len(power_values)))
    numerator = PD + sum(bi_star[i]/(2*ai_star[i]) for i in range(len(power_values)))
    denominator = sum(1/(2*ai_star[i]) for i in range(len(power_values)))
    lambda_new = (numerator/denominator)
    Pg_star = [(lambda_new - bi_star[i])/(2*ai_star[i]) for i in range(len(power_values))]
    epsilon = param[1]
    ...
    if abs(lambda_value - lambda_new) < epsilon:
        ...
        power_values = [Pg_star[i] / (1 - K[i]) for i in range(len(power_values))]
