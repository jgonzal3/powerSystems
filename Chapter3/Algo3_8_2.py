...
def calculate_A_B_coefficients(R,V,delta):
    """
    ...
    a_ij = [(R[i][j] * np.cos(delta[i] - delta[j])) /(V[i] * V[j]) for i in range(NB) for j in range(NB)]
    b_ij = [(R[i][j] * np.sin(delta[i] - delta[j])) /(V[i] * V[j]) for i in range(NB) for j in range(NB)]

    a_coefficients = np.array(a_ij).reshape(NB, NB)
    b_coefficients = np.array(b_ij).reshape(NB, NB)
    return a_coefficients, b_coefficients

def calculate_C_D_coefficients(X,V,delta):
    """
    Calculate the B coefficients for the optimization problem.
    ...
    c_ij = [(X[i][j] * np.cos(delta[i] - delta[j])) /(V[i] * V[j]) for i in range(NB) for j in range(NB)]
    d_ij = [(X[i][j] * np.sin(delta[i] - delta[j])) /(V[i] * V[j]) for i in range(NB) for j in range(NB)]

    c_coefficients = np.array(c_ij).reshape(NB, NB)
    d_coefficients = np.array(d_ij).reshape(NB, NB)
    return c_coefficients, d_coefficients

def calculate_H_A(a_ij, b_ij, c_ij, d_ij, lambda_p, lambda_q, P_injected, Q_injected):

    H = -1*np.ones((2*NG+2,2*NG+2))
    lambda_p = float(lambda_p.item())

    for i in range(NG):
        for j in range(NG):
            if i == j:
                H[i,i] = 2*cost_function[i][0] + 2*lambda_p*a_ij[i,i] + 2*lambda_q*c_ij[i,i]
            else:
                H[i,j] =  lambda_p*(a_ij[i,j] + a_ij[j,i]) + lambda_q*(c_ij[i,j] + c_ij[j,i])

    # line three
    for i in range(NG):
        for k in range(NG,2*NG):
            j = k-NG
            H[i,k] = lambda_p*(b_ij[j,i] - b_ij[i,j]) + lambda_q*(d_ij[j,i] - d_ij[i,j])

    for i in range(NG):
        H[i,2*NG] = sum((a_ij[i,j] + a_ij[j,i])*P_injected[j] + (b_ij[j,i] - b_ij[i,j])*Q_injected[j] for j in range (NB))-1

    for i in range(NG):
        H[i,2*NG+1] = sum((c_ij[i,j] + c_ij[j,i])*P_injected[j] + (d_ij[j,i] - d_ij[i,j])*Q_injected[j] for j in range (NB))

    for j in range(NG):
        for k in range(NG,2*NG):
            i = k-NG
            H[k,j] = lambda_p*(b_ij[i,j] - b_ij[j,i]) + lambda_q*(d_ij[i,j] - d_ij[j,i])

    for k in range(NG,2*NG):
        for l in range(NG,2*NG):
            i = k-NG
            j = l-NG
            H[k,l] = lambda_p*(a_ij[i,j] + a_ij[j,i]) + lambda_q*(c_ij[i,j] + c_ij[j,i])

    for i in range(NG):
        H[i+NG,2*NG] = sum(((a_ij[i,j] + a_ij[j,i])*Q_injected[j] + (b_ij[i,j] - b_ij[j,i])*P_injected[j]) for j in range(NB))

    for i in range(NG):
        H[i+NG,2*NG+1] = sum((c_ij[i,j] + c_ij[j,i])*Q_injected[j] + (d_ij[i,j] - d_ij[j,i])*P_injected[j] for j in range(NB))-1

    for i in range(NG):
        H[2*NG,i] = H[i,2*NG]
        H[2 * NG + 1, i] = H[i, 2 * NG + 1]

    for i in range(NG,2*NG):
        H[2 * NG, i] = H[i, 2 * NG]
        H[2 * NG + 1, i] = H[i, 2 * NG + 1]

    H[6,6] = H[7,7] = H[6,7] = H[7,6] = 0

    return H


# Example usage of read_power_file
def build_jacobian_matrix_A(a_ij, b_ij, c_ij, d_ij, P_injected, Q_injected, P_generated, lambda_p, lambda_q):
    """
    ...
    PL_p = np.sum(a_ij * (np.outer(P_injected, P_injected) + np.outer(Q_injected, Q_injected)) + b_ij * (np.outer(Q_injected, P_injected) - np.outer(P_injected, Q_injected)))
    PL_q = np.sum(c_ij * (np.outer(P_injected, P_injected) + np.outer(Q_injected, Q_injected)) + d_ij * (np.outer(Q_injected, P_injected) - np.outer(P_injected, Q_injected)))

    # Partial of lagrangian with respect to Pg
    for i in range(NG):
        pg[i] =  2 * cost_function[i][0] * P_generated[i] + cost_function[i][1] + \
                 lambda_p*(sum((a_ij[i,j] + a_ij[j,i]) * P_injected[j] + (b_ij[j,i] - b_ij[i,j]) * Q_injected[j] for j in range(NB)) - 1) + \
                 lambda_q*(sum((c_ij[i,j] + c_ij[j,i]) * P_injected[j] + (d_ij[j,i] - d_ij[i,j]) * Q_injected[j] for j in range(NB)))

    for i in range(NG):
        k = i + NG
        pg[k] = lambda_p*(sum((a_ij[i,j] + a_ij[j,i]) * Q_injected[j] + (b_ij[i,j] - b_ij[j,i]) * P_injected[j] for j in range(NB))) + \
                lambda_q*(sum((c_ij[i,j] + c_ij[j,i]) * Q_injected[j] + (d_ij[i,j] - d_ij[j,i]) * P_injected[j] for j in range(NB))-1)

    # Partial of lagrangian with respect to lambda
    pg[2*NG]   = -float(sum(P_generated)) + float(sum(real_power_demand))    + float(PL_p.item())
    pg[2*NG+1] = -float(sum(Q_generated)) + float(sum(reactive_power_demand)) + float(PL_q.item())

    jacobian_matrix = pg.reshape(-1, 1)

    return PL_p, PL_q, jacobian_matrix

def power_flow_estimation_A(a_ij, b_ij, c_ij, d_ij, lambda_p, lambda_q, P_injected, Q_injected, P_generated, Q_generated):
    """
    Function to estimate power flow.
    """
    # Placeholder for the actual implementation

    PL_p, PL_q, B_jacobian = build_jacobian_matrix_A(a_ij, b_ij, c_ij, d_ij, P_injected, Q_injected, P_generated, lambda_p, lambda_q)

    B_hessian = calculate_H_A(a_ij, b_ij, c_ij, d_ij, lambda_p, lambda_q, P_injected, Q_injected)
    ...
    Q_generated = [Q_generated[i] + S[i] for i in range(NG)]
    lambda_p += S[2*NG]
    lambda_q += S[2*NG+1]
    ...
    if squared_sum < tol:
         return lambda_p, lambda_q, P_generated, Q_generated, P_injected,Q_injected, True
    else:
        return lambda_p,lambda_q, P_generated, Q_generated, P_injected,Q_injected, False
...
V[0] = slack_bus[0]
delta[0] = slack_bus[1]
G, B, R_, X_ = np.real(Y), np.imag(Y), np.real(Z), np.imag(Z)

lambda_p = estimate_lambda()
lambda_q =  estimate_lambda()-estimate_lambda()
...
while not converged_II and count_II < R:
    print(f"Iteration_II {count_I}:,lambda_value {lambda_p}, Power {np.round([float(x) for x in Q_generated],6)}")
    assert (P_generated[NG] == P_generated[NG+1] == 0)
    P_injected = P_generated - real_power_demand
    Q_injected = Q_generated - reactive_power_demand
    while not converged_I and count_I < R:
        print(f"Iteration_I {count_I}:,lambda_value {lambda_p}, Power {np.round([float(x) for x in Q_generated],6)}")
    ...
    if (abs(P_generated[0] - real_power_demand[0] - P_injected[0])) > tol:
        ...
        Q_generated[0] = Q_injected[0] + reactive_power_demand[0]
        a_ij, b_ij = calculate_A_B_coefficients(R_, V, delta)
        c_ij, d_ij = calculate_C_D_coefficients(X_, V, delta)
        while not converged_III and count_III < R:
            print(f"Iteration_III {count_III}:,lambda_value {lambda_p}, Power {np.round([float(x) for x in Q_generated],6)}")
            lambda_p, lambda_q, P_generated, Q_generated, P_injected, Q_injected, converged_III = power_flow_estimation_A(a_ij, b_ij,c_ij,d_ij, lambda_p, lambda_q, P_injected, Q_injected, P_generated, Q_generated)
            count_III += 1
        count_III = 0
        converged_III = False
        #print(f"Iteration last:,lambda_value {lambda_p}, Power {np.round([float(x) for x in P_generated],6)}")
