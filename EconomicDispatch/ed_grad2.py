...
while (np.all(gradL > delta) or count < 10):
    ...
    if (np.sum(PG) > PD):
        diff = np.sum(PG) - PD
        loc_of_max = unit_max_inc_cost(diff)
        PG[loc_of_max] = PG[loc_of_max]-diff
        #print(f"PG {np.sum(PG)} is bigger than PD {PD}")
    else:
        diff = PD - np.sum(PG)
        loc_of_min = unit_min_inc_cost(diff)
        PG[loc_of_min] = PG[loc_of_min]+diff
        #print(f"PG {np.sum(PG)} is less than PD {PD}")
