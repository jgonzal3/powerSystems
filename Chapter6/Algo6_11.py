...
def dp_ddelta(i, k, v_current, Y_imag):
    """
    ...
    if i != k:
        return -v_i * v_k * Y_imag[i, k]
    else:
        # Exclude i from the sum using boolean masking
        indices = np.arange(len(v_current)) != i
        v_m = np.abs(v_current[indices])
        Y_im = Y_imag[i, indices]
        return np.sum(v_i * v_m * Y_im)

def dq_dv(i, k, v_current, Y_imag):
    """
    Calculate the partial derivative of P with respect to V for bus i.
    """
    ...
    v_i = np.abs(v_current[i])
    if i != k:
        return -v_i * Y_imag[i, k]
    else:
        indices = np.arange(len(v_current)) != i
        v_m = np.abs(v_current[indices])
        Y_im = Y_imag[i, indices]
        result = 2 * v_i * Y_imag[i, i] + np.sum(v_m * Y_im)
        return -result
...
def estimate_jacobian(v_current, pq_indices, pv_indices, Y_imag):
    """
    ...
    for row, i in enumerate(pq_indices):
        for col, j in enumerate(pq_indices):
            A[row, col] = dp_ddelta(i, j, v_current, Y_imag)

    for row, i in enumerate(qp_indices):
        for col, j in enumerate(qp_indices):
            D[row,col] = dq_dv(i, j, v_current, Y_imag)

    H = np.hstack((np.vstack((A, B)), np.vstack((C, D))))
    return H

def calculate_jacobian(v_current, pq_indices, qp_indices, Y_real):
    """
    Estimate the Jacobian matrix for the power flow equations.
    """


    A = np.array([[dp_ddelta(i, j, v_current, Y_real) for j in pq_indices] for i in pq_indices])
    B = np.array([[0.0 for j in pq_indices] for i in qp_indices])
    C = np.array([[0.0 for j in qp_indices] for i in pq_indices])
    D = np.array([[dq_dv(i, j, v_current, Y_real) for j in qp_indices] for i in qp_indices])
...
while (r < data["R"] and error > data["tol"]):
    ...
    H = calculate_jacobian(v_current, pq_indices, qp_indices, Y.imag)
