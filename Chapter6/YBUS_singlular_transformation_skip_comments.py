...
def get_admitance(file_path):
    ...
    edge_impedance_dict = {edge: impedance for edge, impedance in zip(edges, Y)}
    ...
    for i in range(no_nodes + no_edges):
        Y_n[i][i] = 0.0 if i < no_nodes else Y_n[i][i] * (
                1 / Y[i - no_nodes] + line_charging[i - no_nodes][i - no_nodes])
    ...
    return (admitance_matrix)# + shunt_matrix)
